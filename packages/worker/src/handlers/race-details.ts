...
export async function handleRaceDetails(request: Request, env: Env): Promise<Response> {
  try {
    ...
    if (storedResults.length === 0 && new Date(race.date as string) < new Date()) {
      try {
        const year = new Date(race.date as string).getFullYear()
        const liveResults = await fastf1Client.getRaceResults(year, race.circuit as string)
    ...
    if (storedQualifying.length === 0 && new Date(race.date as string).getTime() - Date.now() < 3 * 24 * 60 * 60 * 1000) {
      try {
        const year = new Date(race.date as string).getFullYear()
        const liveQualifying = await fastf1Client.getQualifyingResults(year, race.circuit as string)
...
async function getWeatherForRace(circuit: string, date: string) {
  ...
  const coord = coordinates[circuit as keyof typeof coordinates]
  if (!coord) {
...
function getTypicalWeather(circuit: string) {
  const patterns: Record<string, { temp: number; rain: number; conditions: string }> = {
...
export async function handleQualifyingData(request: Request, env: Env): Promise<Response> {
  try {
    const url = new URL(request.url)
