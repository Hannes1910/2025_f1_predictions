export async function handleCreateModelMetrics(request, env) {
    try {
        // Check API key
      ...
      VALUES (?, ?, ?, ?, ?)
    `).bind(data.model_version, data.race_id || null, data.mae, data.accuracy, data.created_at || new Date().toISOString()).run();
...
export async function handleGetModelMetrics(request, env) {
    try {
    ...
    `).all();
        ...
        for (const metric of results) {
            const version = metric.model_version;
