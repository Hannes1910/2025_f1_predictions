 ...
 */
export async function handleGenerateUltraPredictions(request, env) {
    try {
        ...
        const { results: races } = await env.DB.prepare(`SELECT id, name, date FROM races 
       WHERE date >= date('now') 
       ORDER BY date ASC 
       LIMIT 1`).all();
        if (races.length === 0) {
 ...
 */
export async function handleLatestUltraPredictions(request, env) {
    try {
        // Get the latest race with ultra predictions
        const { results: races } = await env.DB.prepare(`SELECT DISTINCT r.id, r.name, r.date, r.circuit
       FROM races r
       ...
       ORDER BY p.predicted_position`).bind(race.id).all();
        // Get model status
        const predictor = new HybridPredictor(env.ML_SERVICE_URL);
 ...
 */
export async function handleModelStatus(request, env) {
    try {
        const predictor = new HybridPredictor(env.ML_SERVICE_URL);
        const modelStatus = await predictor.getModelStatus();
        // Get recent model metrics from database
        const { results: metrics } = await env.DB.prepare(`SELECT * FROM model_metrics 
       ORDER BY created_at DESC 
       LIMIT 10`).all();
        // Get prediction counts by model version
        const { results: predictionStats } = await env.DB.prepare(`SELECT 
        model_version,
       ...
       ORDER BY last_prediction DESC`).all();
        return new Response(JSON.stringify({
 ...
 */
async function storeUltraPredictions(env, predictionResult) {
    const { race_id, predictions, model_version } = predictionResult;
    // Clear existing predictions for this race and model version
    await env.DB.prepare(`DELETE FROM predictions 
     WHERE race_id = ? AND model_version = ?`).bind(race_id, model_version).run();
    ...
    for (const pred of predictions) {
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(pred.race_id, pred.driver_id, pred.predicted_position, 75.0 + pred.predicted_position, // Mock lap time
        pred.confidence, pred.model_version, pred.created_at).run();
    }
 ...
 */
async function updateModelMetrics(env, modelVersion, accuracy) {
    try {
        await env.DB.prepare(`INSERT INTO model_metrics (
        model_name, accuracy, mae, version, created_at
      ) VALUES (?, ?, ?, ?, ?)`).bind('ultra_predictor', accuracy, 2.0 - (accuracy - 0.8) * 5, // Estimated MAE based on accuracy
        modelVersion, new Date().toISOString()).run();
